...
발표용으로 바로 실행할 수 있도록 `data/scores.csv` 파일을 기본 데이터로 사용할 수 있습니다.
앱을 열면 사이드바의 `데이터 선택`에서 `내장 성적 데이터`, `업로드 데이터`, `가상 예시 데이터` 중 하나를 선택할 수 있습니다.

Public GitHub 저장소에는 개인정보가 포함된 원본 성적표를 올리지 마세요. 실제 성적표를 반영할 때는 이름, 학번, 반, 번호 등을 모두 제거하고 아래처럼 `점수` 열만 남겨 `data/scores.csv`를 교체하면 됩니다.
